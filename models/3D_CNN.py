...
def inference(boxes, dataconfig):       
    ...
    with tf.variable_scope('conv1') as scope:                                                          # name of the block  
    ...
    with tf.variable_scope('local3') as scope:                                     # FULLY CONNECTED LAYER 
    ...
    with tf.variable_scope('local4') as scope:                                      # ANOTHER FULLLY CONNECTED LAYER     
        dim = np.prod(prev_layer.get_shape().as_list()[1:])
        prev_layer_flat = tf.reshape(prev_layer, [-1, dim])
        weights = _weight_variable('weights', [dim, FC_SIZE])
        biases = _bias_variable('biases', [FC_SIZE])
    ...
    with tf.variable_scope('softmax_linear') as scope:                              # SOFTMAX LAYER 
        dim = np.prod(prev_layer.get_shape().as_list()[1:])
